...
def search_youtube_videos(query, max_results=10, user_details=None):
    ...
    try:
        # Build query with user-specific parameters
        ...
        search_request = youtube.search().list(
            ...
            part="snippet",
            type="video",
            maxResults=max_results * 3,  # Get more to filter down
        ...
        for item in videos_response.get("items", []):
            ...
            # Duration check (7-30 minutes)
            duration = item["contentDetails"]["duration"]
            duration_sec = parse_youtube_duration(duration)
            if not (420 <= duration_sec <= 1800):  # 7-30 minutes
                continue
...
def apply_advanced_filters(videos, user_details):
    ...
    for video in videos:
        ...
        # Recency (more recent videos get higher score)
        try:
