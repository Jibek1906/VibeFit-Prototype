...
def workouts_api(request):
    ...
    # Check cache first - using more specific cache key
    ...
    if is_rest_day:
        ...
        cache.set(cache_key, response, 3600 * 6)  # Cache for 6 hours
        return JsonResponse(response)

    # Generate workout queries based on user details and date
    ...
    # If we don't have enough videos, try fallback queries
...
def is_video_available(video_id):
    ...
    # If not in cache, assume it's available (actual check happens in youtube_api)
    return True
...
def calculate_rest_day(user_details, date):
    ...
    # Count rest days already in this week
    for i in range(7):
    ...
    if is_scheduled_rest_day(user_details, date):
        # Enforce max 2 rest days per week
...
def generate_workout_queries(user_details, date):
    ...
    # Determine workout focus based on user goal and day of week
    ...
    # Generate 5 different workout queries with variations
    for i in range(5):
        ...
        # Remove time-based variations since we don't have time info
        # Instead add variation based on iteration count
        if i % 2 == 0:
            query += " full session"
        else:
            query += " quick workout"
...
def get_workout_tags(title, user_details):
    ...
    return list(set(tags))[:4]  # Limit to 4 unique tags
